...
public class KickCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (commandLabel.equalsIgnoreCase("kick")) {
			...
			db.addBan(victim.getUniqueId(), SeruBans.KICK, 0,
					mod.getUniqueId(), reason, display);

			// prints to players on server with perms
			plugin.printServer(
					plugin.text().GlobalMessage(plugin.GlobalKickMessage,
							reason, mod.getName(), victim.getName()), silent);
			// logs its
			...
			victim.kickPlayer(plugin.text().GetColor(
					plugin.text().PlayerMessage(plugin.KickMessage, reason,
							mod.getName())));
			// adds player to db
			return true;
		}
